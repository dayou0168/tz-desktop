		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"TZ.exe") && exeName != L"TZ.exe") {
	...
	for (size_t i = 0; i < from.size(); ++i) {
		...
		do {
			...
			if (!copyResult) {
				...
				WCHAR errMsg[2048];
				wsprintf(errMsg, L"Failed to update TZ :(\n%s is not accessible.", tofname.c_str());
...
void updateRegistry() {
	if (versionNum && versionNum != 0x7FFFFFFF) {
		...
		HKEY rkey;
		LSTATUS status = RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{C00EF9CB-D985-4543-84B2-B606F6E61F6C}_is1", 0, KEY_QUERY_VALUE | KEY_SET_VALUE, &rkey);
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								RegDeleteValue(rkey, L"HelpLink");
								RegDeleteValue(rkey, L"URLInfoAbout");
								RegDeleteValue(rkey, L"URLUpdateInfo");
							}
						}
					}
				}
...
static const WCHAR *_programName = L"TZ"; // folder in APPDATA, if current path is unavailable for writing
