...
#ifdef OS_WIN_STORE
		return appData.absolutePath() + u"/TZ UWP/"_q;
#else // OS_WIN_STORE
...
void psDoFixPrevious() {
	try {
		...
		if (SUCCEEDED(userDesktopRes)) {
			userDesktopLnk = QString::fromWCharArray(userDesktopFolder) + "\\TZ.lnk";
		}
		if (SUCCEEDED(commonDesktopRes)) {
			commonDesktopLnk = QString::fromWCharArray(commonDesktopFolder) + "\\TZ.lnk";
		}
